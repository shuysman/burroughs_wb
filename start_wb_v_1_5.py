...
def find_file_chunks(search_first_year, search_last_year, textfilename = 'null'): # For parsing filenames of GCM data
    ...
    if model == "historical":
        ## Scenario is 4 for gridmet, 5 for projections
        ## this is hideous, but I set the file names up
...
if __name__ == '__main__':
    ...
    slope = gdal.Open(burroughs_data_path + "planting_unit_slope.tif").ReadAsArray()
    aspect = gdal.Open(burroughs_data_path + "planting_unit_aspect.tif").ReadAsArray()
    ...
    snow_thresh_temperatures = gdal.Open(burroughs_data_path + "jennings_t50_coefficients.tif").ReadAsArray()
