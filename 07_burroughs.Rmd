...
stats <- data.frame(elev_mean = zonal(elev, vect(units), fun = "mean")[[1]],
                    ...
                    aspect_max = zonal(aspect, vect(units), fun = "max", na.rm = TRUE)[[1]],
                    aspect_min = zonal(aspect, vect(units), fun = "min", na.rm = TRUE)[[1]]
                    )
...
aet_scale <-
    scale_fill_viridis_c(option = "D", limits = c(72, 631))

cwd_scale <-
  scale_fill_viridis_c(option = "B", limits = c(0, 400))

aet_sd_scale <-
    scale_fill_viridis_c(option = "A", limits = c(7, 101))

cwd_sd_scale <-
    scale_fill_viridis_c(option = "C", limits = c(0, 97))
```
...
zonal(soil, vect(units), fun = "max", na.rm = TRUE) %>%
    cbind(Unit = units$Unit, .) %>%
  knitr::kable(digits = 2, col.names = c("Unit", "Max Soil WHC (mm)"), caption = "Max soil WHC (mm)")

zonal(soil, vect(units), fun = "min", na.rm = TRUE) %>%
    cbind(Unit = units$Unit, .) %>%
    knitr::kable(digits = 2, col.names = c("Unit", "Mean Min WHC (mm)"), caption = "Min soil WHC (mm)")
```
...
ggplot(wbp_points) +
  ...
  geom_point(shape = "+", size = 12, data = filter(wbp_points, park == "burroughs"), aes(x = CWD, y = AET), color = "black") +
  geom_point(shape = "+", size = 12, data = filter(wbp_points, park == "burroughs"), aes(x = CWD_45, y = AET_45), color = "orange") +
  geom_point(shape = "+", size = 12, data = filter(wbp_points, park == "burroughs"), aes(x = CWD_85, y = AET_85), color = "red") +
...
aet_2002_plot <- ggplot() +
    ...
    geom_sf(data = units, fill = NA, lwd = 1.5) +
    geom_sf_label(data = units, aes(label = Unit), fill = NA) +
    labs(title = "1982-2002", fill = "AET (mm)") +
...
cwd_2002_plot <- ggplot() +
    ...
    geom_sf(data = units, fill = NA, lwd = 1.5) +
    geom_sf_label(data = units, aes(label = Unit), fill = NA) +
    labs(title = "1982-2002", fill = "CWD (mm)") +
    annotation_north_arrow(aes(location = "tr"), height = arrow_h, width = arrow_w) +
    annotation_scale(aes(location = "br"))
...
aet_2022_plot <- ggplot() +
    ...
    geom_sf(data = units, fill = NA, lwd = 1.5) +
    geom_sf_label(data = units, aes(label = Unit), fill = NA) +
    labs(title = "2002-2022", x = "", y = "", fill = "AET (mm)") +
    annotation_north_arrow(aes(location = "tr"), height = arrow_h, width = arrow_w) +
    annotation_scale(aes(location = "br"))
...
cwd_2022_plot <- ggplot() +
    ...
    geom_sf(data = units, fill = NA, lwd = 1.5) +
    geom_sf_label(data = units, aes(label = Unit), fill = NA) +
    labs(title = "2002-2022", x = "", y = "", fill = "CWD (mm)") +
    annotation_north_arrow(aes(location = "tr"), height = arrow_h, width = arrow_w) +
    annotation_scale(aes(location = "br"))
...
aet_1988_plot <- ggplot() +
    ...
    geom_sf(data = units, fill = NA, lwd = 1.5) +
    geom_sf_label(data = units, aes(label = Unit), fill = NA) +
    labs(title = "1988", x = "", y = "", fill = "AET (mm)") +
    annotation_north_arrow(aes(location = "tr"), height = arrow_h, width = arrow_w) +
    annotation_scale(aes(location = "br"))
...
cwd_1988_plot <- ggplot() +
    ...
    geom_sf(data = units, fill = NA, lwd = 1.5) +
    geom_sf_label(data = units, aes(label = Unit), fill = NA) +
    labs(title = "1988", x = "", y = "", fill = "CWD (mm)") +
    annotation_north_arrow(aes(location = "tr"), height = arrow_h, width = arrow_w) +
    annotation_scale(aes(location = "br"))
```
...
aet_warm_wet_plot <- ggplot() +
  geom_spatraster(data = clamp(aet_warm_wet, lower = 1, values = NA), alpha = 1) +
  aet_scale +
  geom_sf(data = units, fill = NA, lwd = 1.5) +
  geom_sf_label(data = units, aes(label = Unit), fill = NA) +
  labs(title = "2075–2099 Warm-Wet (MRI-CGCM3 RCP8.5)", x = "", y = "", fill = "AET (mm)") +
    annotation_north_arrow(aes(location = "tr"), height = arrow_h, width = arrow_w) +
  annotation_scale(aes(location = "br"))

cwd_warm_wet_plot <- ggplot() +
    geom_spatraster(data = clamp(cwd_warm_wet, lower = 1, values = NA), alpha = 1) +
    cwd_scale +
    geom_sf(data = units, fill = NA, lwd = 1.5) +
    geom_sf_label(data = units, aes(label = Unit), fill = NA) +
    labs(title = "2075–2099 Warm-Wet (MRI-CGCM3 RCP8.5)", x = "", y = "", fill = "CWD (mm)") +
    annotation_north_arrow(aes(location = "tr"), height = arrow_h, width = arrow_w) +
    annotation_scale(aes(location = "br"))

aet_hot_wet_plot <- ggplot() +
    geom_spatraster(data = clamp(aet_hot_wet, lower = 1, values = NA), alpha = 1) +
    aet_scale +
    geom_sf(data = units, fill = NA, lwd = 1.5) +
    geom_sf_label(data = units, aes(label = Unit), fill = NA) +
    labs(title = "2075–2099 Hot-Wet (CanESM2 RCP8.5)", x = "", y = "", fill = "AET (mm)") +
    annotation_north_arrow(aes(location = "tr"), height = arrow_h, width = arrow_w) +
    annotation_scale(aes(location = "br"))

cwd_hot_wet_plot <- ggplot() +
    geom_spatraster(data = clamp(cwd_hot_wet, lower = 1, values = NA), alpha = 1) +
    cwd_scale +
    geom_sf(data = units, fill = NA, lwd = 1.5) +
    geom_sf_label(data = units, aes(label = Unit), fill = NA) +
    labs(title = "2075–2099 Hot-Wet (CanESM2 RCP8.5)", x = "", y = "", fill = "CWD (mm)") +
    annotation_north_arrow(aes(location = "tr"), height = arrow_h, width = arrow_w) +
    annotation_scale(aes(location = "br"))

aet_warm_dry_plot <- ggplot() +
    geom_spatraster(data = clamp(aet_warm_dry, lower = 1, values = NA), alpha = 1) +
    aet_scale +
    geom_sf(data = units, fill = NA, lwd = 1.5) +
    geom_sf_label(data = units, aes(label = Unit), fill = NA) +
    labs(title = "2075–2099 Warm-Dry (MRI-CGCM3 RCP4.5)", x = "", y = "", fill = "AET (mm)") +
    annotation_north_arrow(aes(location = "tr"), height = arrow_h, width = arrow_w) +
    annotation_scale(aes(location = "br"))

cwd_warm_dry_plot <- ggplot() +
    geom_spatraster(data = clamp(cwd_warm_dry, lower = 1, values = NA), alpha = 1) +
    cwd_scale +
    geom_sf(data = units, fill = NA, lwd = 1.5) +
    geom_sf_label(data = units, aes(label = Unit), fill = NA) +
    labs(title = "2075–2099 Warm-Dry (MRI-CGCM3 RCP4.5)", x = "", y = "", fill = "CWD (mm)") +
    annotation_north_arrow(aes(location = "tr"), height = arrow_h, width = arrow_w) +
    annotation_scale(aes(location = "br"))

aet_hot_dry_plot <- ggplot() +
    geom_spatraster(data = clamp(aet_hot_dry, lower = 1, values = NA), alpha = 1) +
    aet_scale +
    geom_sf(data = units, fill = NA, lwd = 1.5) +
    geom_sf_label(data = units, aes(label = Unit), fill = NA) +
    labs(title = "2075–2099 Hot-Dry (HadGEM2-CC365 RCP8.5)", x = "", y = "", fill = "AET (mm)") +
    annotation_north_arrow(aes(location = "tr"), height = arrow_h, width = arrow_w) +
    annotation_scale(aes(location = "br"))

cwd_hot_dry_plot <- ggplot() +
    geom_spatraster(data = clamp(cwd_hot_dry, lower = 1, values = NA), alpha = 1) +
    cwd_scale +
    geom_sf(data = units, fill = NA, lwd = 1.5) +
    geom_sf_label(data = units, aes(label = Unit), fill = NA) +
    labs(title = "2075–2099 Hot-Dry (HadGEM2-CC365 RCP8.5)", x = "", y = "", fill = "CWD (mm)") +
    annotation_north_arrow(aes(location = "tr"), height = arrow_h, width = arrow_w) +
    annotation_scale(aes(location = "br"))
...
``` {r, combined-aet-plot, fig.height = 9, fig.width = 11}
ggarrange(aet_1988_plot, aet_2022_plot,
          aet_hot_wet_plot, aet_warm_dry_plot,
          common.legend = TRUE
          )

```

``` {r, combined-cwd-plot, fig.height = 9, fig.width = 11}
ggarrange(cwd_1988_plot, cwd_2022_plot,
          cwd_warm_dry_plot, cwd_hot_dry_plot,
          common.legend = TRUE
          )
```
...
``` {r, projections-sd-aet, warning = FALSE, message = FALSE, echo = FALSE, fig.height = 9, fig.width = 11, fig.cap = "AET Interannual variability (standard deviation) in climatic conditions under projections from selected GCMs representing hot/wet, hot/dry, warm/wet, and warm/dry conditions"}
...
``` {r, projections-sd-cwd, warning = FALSE, message = FALSE, echo = FALSE, fig.height = 9, fig.width = 11, fig.cap = "CWD Interannual variability (standard deviation) in climatic conditions under projections from selected GCMs representing hot/wet, hot/dry, warm/wet, and warm/dry conditions"}
...
``` {r, best-plantings-setup1, eval = FALSE, include = FALSE, warning = FALSE, echo = FALSE, message = FALSE}
