...
water balance for the Burroughs Creek site \@ref(fig:planting-units) in Shoshone National
...
strongly affect modeled water balance [@dyerGISBasedWaterBalance2019].
Our objective was to use historical and projected climate data to model the
climatic water balance of the Burroughs Creek site using the highest
resolution topographic and soil survey data available to understand
fine-scale spatial variation in the climatic water balance across the site in
order to guide planting of whitebark pine seedlings.

Depending on management objects, these results can be interpreted in
different ways.  For example, if management goals are to:
...
LiDAR digital elevation model (DEM) provided by the USGS
3DEP [@u.s.geologicalsurveyWYFEMAEast2020].  Slope and aspect were
...
database
[@naturalresourcesconservationserviceSoilSurveyGeographic2015].  A 1
m raster was generated from the WHC data in QGIS using the 1 m DEM
raster as a reference for sampling grid cells.  

Historical climate data (maximum temperature, minimum temperature,
precipitation) from 1979-2022 was provided by the gridMET
dataset at a daily timestep and 4 km resolution
...
the MACA data set were included in this analysis:
| GMC            |
|----------------|
| bcc-csm1-1-m   |
| bcc-csm1-1     |
...
Deficit (mm)
were calculated for the historical dataset as well as for each
GCM/Scenario combination.  Ensemble averages for annual sums of AET
and Deficit were created.
...
``` {r historical2, warning = FALSE, message = FALSE, echo = FALSE, fig.height = 8, fig.width = 16, out.width = '100%'}
...
``` {r projections, warning = FALSE, message = FALSE, echo = FALSE, fig.height = 8, fig.width = 10, out.width = '100%'}
current <- 2006:2024
near <- 2025:2049
mid <- 2050:2074
end <- 2075:2099
...
ggplot() +
  ...
  cwd_sd_scale +
  labs(title = "RCP4.5 Interannual Variability in ensemble mean annual CWD, End-Century (2075-2099)")

ggplot() +
  geom_spatraster(data = cwd_ens85_end_sd) +
  cwd_sd_scale +
  labs(title = "RCP8.5 Interannual Variability in ensemble mean annual CWD, End-Century (2075-2099)")

### AET Historical Plots --------------------
...
##    scale_fill_gradientn(colors = pal_greys, na.value = NA) +
    ...
    geom_sf(data = units, fill = NA, lwd = 1.5) +
    geom_sf_label(data = units, aes(label = Unit), fill = NA) +
    labs(title = "RCP4.5 Ensemble Mean AET, Near-future (2025-2049)") +
    annotation_north_arrow(aes(location = "tr")) +
    annotation_scale(aes(location = "br"))
...
##    scale_fill_gradientn(colors = pal_greys, na.value = NA) +
    ...
    geom_sf(data = units, fill = NA, lwd = 1.5) +
    geom_sf_label(data = units, aes(label = Unit), fill = NA) +
    labs(title = "RCP4.5 Ensemble Mean AET, Mid-century (2050-2074)") +
    annotation_north_arrow(aes(location = "tr")) +
    annotation_scale(aes(location = "br"))
...
ggplot() +
    ...
    geom_sf(data = units, fill = NA, lwd = 1.5) +
    geom_sf_label(data = units, aes(label = Unit), fill = NA) +
    labs(title = "RCP4.5 Ensemble Mean AET, End-century (2075-2099)") +
    annotation_north_arrow(aes(location = "tr")) +
  annotation_scale(aes(location = "br"))
...
``` {r baseline, warning = FALSE, message = FALSE, echo = FALSE, fig.height = 8, fig.width = 16, out.width = '100%'}
## CWD change from baseline

cwd_rcp45_change_from_baseline <- cwd_ens45_end - cwd_2022
(global(mean(cwd_rcp45_change_from_baseline)))
...
##    scale_fill_gradientn(colors = pal_greys, na.value = NA) +
    ...
    geom_sf(data = units, fill = NA, lwd = 1.5) +
    geom_sf_label(data = units, aes(label = Unit), fill = NA) +
    labs(title = "RCP4.5 Ensemble Mean CWD end-of-century (2075-2099) change from baseline (2002-2022)") +
    annotation_north_arrow(aes(location = "tr")) +
    annotation_scale(aes(location = "br"))

cwd_rcp85_change_from_baseline <- cwd_ens85_end - cwd_2022
(global(mean(cwd_rcp85_change_from_baseline)))
...
##    scale_fill_gradientn(colors = pal_greys, na.value = NA) +
    ...
    geom_sf(data = units, fill = NA, lwd = 1.5) +
    geom_sf_label(data = units, aes(label = Unit), fill = NA) +
    labs(title = "RCP8.5 Ensemble Mean CWD end-of-century (2075-2099) change from baseline (2002-2022)") +
    annotation_north_arrow(aes(location = "tr")) +
    annotation_scale(aes(location = "br"))
...
(global(mean(aet_rcp45_change_from_baseline)))
...
##    scale_fill_gradientn(colors = pal_greys, na.value = NA) +
    ...
    geom_sf(data = units, fill = NA, lwd = 1.5) +
    geom_sf_label(data = units, aes(label = Unit), fill = NA) +
    labs(title = "RCP4.5 Ensemble Mean AET end-of-century (2075-2099) change from baseline (2002-2022)") +
    annotation_north_arrow(aes(location = "tr")) +
    annotation_scale(aes(location = "br"))


aet_rcp85_change_from_baseline <- aet_ens85_end - aet_2022
(global(mean(aet_rcp85_change_from_baseline)))
...
## years = c(near, mid, end)
## vars = c("aet", "cwd")
## types = c("mean", "min", "max")
## secnarios <- c("rcp45", "rcp85")
...
## Best Planting Sites
### Historical
``` {r best-plantings, warning = FALSE, message = FALSE, echo = FALSE, fig.height = 8, fig.width = 10, out.width = '100%'}
n_sites <- 1000000
...
``` {r best-45, warning = FALSE, message = FALSE, echo = FALSE, fig.height = 8, fig.width = 10, out.width = '100%'}
highest_aet <- selectHighest(aet_ens45_end, n = n_sites)
lowest_cwd <- selectHighest(cwd_ens45_end, n = n_sites, low = TRUE)
...
(mean_aet <- global(mask(aet_ens45_end, highest_aet), fun = "mean", na.rm = TRUE))
(mean_cwd <- global(mask(cwd_ens45_end, lowest_cwd), fun = "mean", na.rm = TRUE))
...
##    geom_spatraster(data = subset(tmmn$daily_minimum_temperature, 1)) +
    ...
    geom_sf(data = units, fill = NA, lwd = 1.5) +
    geom_sf_label(data = units, aes(label = Unit), alpha = 0.4) +
    labs(title = glue::glue("Best Planting Sites (Highest AET + Lowest Deficit RCP 45 End of Century) n = {n_best_sites}, {round(n_best_sites * 100 / ncell(elev), 2)}%")) +
    annotation_north_arrow(aes(location = "tr")) +
    annotation_scale(aes(location = "br"))
...
``` {r best-85, warning = FALSE, message = FALSE, echo = FALSE, fig.height = 8, fig.width = 10, out.width = '100%'}
highest_aet <- selectHighest(aet_ens85_end, n = n_sites)
lowest_cwd <- selectHighest(cwd_ens85_end, n = n_sites, low = TRUE)
...
(mean_aet <- global(mask(aet_ens85_end, highest_aet), fun = "mean", na.rm = TRUE))
(mean_cwd <- global(mask(cwd_ens85_end, lowest_cwd), fun = "mean", na.rm = TRUE))
...
##    geom_spatraster(data = subset(tmmn$daily_minimum_temperature, 1)) +
    ...
    geom_sf(data = units, fill = NA, lwd = 1.5) +
    geom_sf_label(data = units, aes(label = Unit), alpha = 0.4) +
    labs(title = glue::glue("Best Planting Sites (Highest AET + Lowest Deficit RCP 85 End of Century) n = {n_best_sites}, {round(n_best_sites * 100 / ncell(elev), 2)}%")) +
    annotation_north_arrow(aes(location = "tr")) +
    annotation_scale(aes(location = "br"))
